...
const chatRecordToChat = (record: ChatRecord): Chat => {
  ...
  if (isGroup) {
    ...
    name =
      record.other_user?.full_name || record.other_user?.username || "User";
  }
  ...
  return {
    ...
    ticks: record.last_message_status ?? null,
    otherUserId: record.other_user?.id,
...
export default function ChatsScreen() {
  ...
  const [section, setSection] =
    useState<(typeof CHAT_SECTIONS)[number]>("Chats");
  ...
  useRealtimeRecovery(loadChats);

  useEffect(() => {
    if (!currentUserId) return;
    const channel = supabase
      .channel(`chat-list-updates:${currentUserId}`)
      .on(
        "postgres_changes",
      ...
      .subscribe((status) => {
        if (status === "SUBSCRIBED") void loadChats();
      });
    return () => {
  ...
  const realtimeChatKey = chats
    .map((chat) => `${chat.id}:${chat.otherUserId ?? ""}`)
    .sort()
    .join("|");
  useEffect(() => {
    if (!currentUserId || !realtimeChatKey) return;
    const channelRows = chats.map((chat) => ({
      id: chat.id,
      otherUserId: chat.otherUserId,
    }));
    const channels = channelRows.map(({ id: chatId, otherUserId }) => {
      ...
      channel
        .on("presence", { event: "sync" }, () => {
          const online = Boolean(
            otherUserId &&
            Object.keys(channel.presenceState()).includes(otherUserId),
          );
          setChats((items) =>
            items.map((item) =>
              item.id === chatId && item.online !== online
                ? { ...item, online }
                : item,
            ),
          );
        ...
        .on("broadcast", { event: "typing" }, ({ payload }) => {
          ...
          setChats((items) =>
            items.map((item) =>
              item.id === chatId
                ? { ...item, typing: Boolean(event.isTyping) }
                : item,
            ),
          );
        })
    ...
    // realtimeChatKey intentionally prevents reconnecting channels for presence-only state changes.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentUserId, realtimeChatKey]);
  ...
  return (
    <View style={styles.container}>
        ...
        <View style={styles.sectionTabs}>
          {CHAT_SECTIONS.map((option) => (
            <TouchableOpacity
              ...
              style={[
                styles.sectionTab,
                section === option && styles.sectionTabActive,
              ]}
              activeOpacity={0.8}
            >
              <Text
                style={[
                  ...
                  section === option && styles.sectionTabTextActive,
                ]}
              >
                {option}
              </Text>
        ...
        {loading ? (
          ...
          <View style={styles.centerContainer}>
            ...
            <Text style={styles.emptyTitle}>
              {section === "Groups" ? "No groups yet" : "No chats yet"}
            </Text>
            ...
            >
              ...
              <Text style={styles.emptyButtonText}>
                {section === "Groups" ? "New Group" : "New Chat"}
              </Text>
            </TouchableOpacity>
          </View>
        ) : (
      ...
      <TouchableOpacity
        style={styles.fab}
        activeOpacity={0.9}
        onPress={() => router.push("/(public)/new_chat")}
      >
        <GradientWrapper
          colors={["#4361EE", "#7955D9"]}
...
function ChatRow({
  ...
  const TickIcon =
    chat.ticks === "read" || chat.ticks === "delivered"
      ? CheckCheck
      : chat.ticks === "sent"
        ? Check
        : null;
  ...
  return (
    <TouchableOpacity
      style={[styles.chatRow, chat.pinned && styles.chatRowPinned]}
      activeOpacity={0.75}
      onPress={onPress}
    >
      <View style={styles.avatarWrap}>
        {chat.hasUnseenStatus ? (
          <GradientWrapper
            colors={STATUS_RING}
            style={chat.isGroup ? styles.groupRing : styles.avatarRing}
          >
            <View
              style={
                chat.isGroup
                  ? styles.groupAvatarInnerWrap
...
function ChatAvatar({
  ...
  const wrapper = bare
    ? [shape, { backgroundColor: chat.color }]
    : [shape, { backgroundColor: chat.color }];
  if (chat.isGroup) {
    return (
      <View style={wrapper}>
