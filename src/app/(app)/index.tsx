...
export default function PulseHomeScreen() {
  ...
  const visiblePulses = pulses.filter((pulse) =>
    `${pulse.author} ${pulse.channel} ${pulse.text}`
      .toLowerCase()
  ...
  const loadHomeData = useCallback(async () => {
    ...
    try {
      ...
      const friendStories = storyRows
        .filter((story: any) => story.author_id !== currentProfile.id)
        .map((story: any, index) => {
          const friendName =
            story.profiles?.full_name?.trim() ||
      ...
      setPulses(
        feedRows.map((post: any, index: number) => {
          const author =
            post.profiles?.full_name?.trim() ||
            post.profiles?.username ||
            "Universal user";
          const liked = (post.likes ?? []).some(
            (like: any) => like.user_id === currentProfile.id,
          );
          const comment = post.comments?.[0];
          return {
            id: post.id,
            author,
            initials: getDisplayInitials(author),
            avatar: post.profiles?.avatar_url || undefined,
            ago: new Date(post.created_at).toLocaleDateString(),
            text: post.content || "",
      ...
      if (loadId === latestHomeLoad.current)
  ...
  const toggleReaction = async (id: string, type: ReactionType) => {
    ...
    if (!current) return;
    try {
      await togglePostLike(id, Boolean(current.reaction));
    } catch (error) {
      console.warn("[home] like failed", error);
      return;
    }
    setPulses((items) =>
      items.map((pulse) => {
        if (pulse.id !== id) return pulse;
        if (pulse.reaction === type)
          return { ...pulse, reaction: null, likes: pulse.likes - 1 };
        return {
          ...pulse,
          reaction: type,
          likes: pulse.reaction ? pulse.likes : pulse.likes + 1,
        };
      }),
    );
  };
  ...
  const markStorySeen = (story: Story) => {
    if (story.own) return;
    setStories((items) =>
      items.map((s) => (s.id === story.id ? { ...s, seen: true } : s)),
    );
  };
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={["top"]}>
            ...
            <View style={styles.header}>
              ...
              <View style={styles.brand}>
                <View style={styles.brandRow}>
                  ...
                  <Text style={styles.brandName}>Universal</Text>
                </View>
                <Text style={styles.brandSub}>Universal Chat</Text>
              </View>
              <TouchableOpacity
                style={styles.inboxButton}
                onPress={() => router.push("/(chat)/Main_chat")}
              >
            ...
            {searching ? (
              <View style={styles.searchBox}>
                ...
                <TextInput
                  ...
                  placeholder="Find people, circles, ideas"
            ...
            <View style={styles.actionRow}>
              ...
              >
                ...
                <Text style={styles.searchActionText}>
                  {searching ? "Close search" : "Explore your circles"}
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={styles.scheduleAction}
                onPress={() => router.push("/(app)/scheduling")}
              >
                <Sparkles size={17} color={theme.secondary} />
              </TouchableOpacity>
            </View>

            <View style={styles.storiesHeader}>
              <Text style={styles.sectionTitle}>Moments</Text>
              <Text style={styles.sectionLink}>Close friends</Text>
            </View>
            <FlatList
              horizontal
              data={stories}
            ...
            <View style={styles.feedTitleRow}>
              <View>
                ...
                <Text style={styles.feedHint}>
                  Fresh from the people and channels you follow
                </Text>
              </View>
              <TouchableOpacity>
        ...
        renderItem={({ item, index }) => (
          <PulseCard
            ...
            onComment={async (content) => {
              await addPostComment(item.id, content);
              setPulses((rows) =>
                rows.map((row) =>
                  row.id === item.id
                    ? {
        ...
        ListEmptyComponent={
          <View style={styles.emptyFeedCard}>
            <Text style={styles.emptyFeedTitle}>
              Your feed is ready for your first update
            </Text>
            <Text style={styles.emptyFeedText}>
              Once you add friends and connect with people, their activity will
              appear here.
            </Text>
          </View>
        }
      ...
      >
        <GradientWrapper colors={gradient} style={styles.fabGradient}>
          <Plus size={25} color="#fff" strokeWidth={2.6} />
        </GradientWrapper>
      </TouchableOpacity>
      ...
      <SharePicker
        ...
        onSelect={async (recipientId) => {
          ...
          if (shareTarget.kind === "post")
            await sharePost(shareTarget.id, recipientId);
          else await shareStory(shareTarget.id, recipientId);
          setShareTarget(null);
        }}
      />
    </SafeAreaView>
  ...
  const save = async () => {
    ...
    try {
      const mediaUrl = media
        ? await uploadSocialMedia(media.uri, media.type)
        : null;
      await createPost({
        content: text,
        mediaUrl,
        ...
        >
          ...
          <View style={{ flexDirection: "row", gap: 10 }}>
            ...
            >
              ...
              <Text
                style={{ color: theme.text, fontFamily: Fonts?.sansSemiBold }}
              >
                Add photo
              </Text>
            </TouchableOpacity>
            <TouchableOpacity
              onPress={save}
              disabled={saving || (!text.trim() && !media)}
              style={{
                flex: 1,
                height: 48,
                borderRadius: 16,
                ...
                alignItems: "center",
                justifyContent: "center",
                opacity: saving || (!text.trim() && !media) ? 0.55 : 1,
              }}
            >
    ...
    if (!result.canceled && result.assets[0])
      setMedia({
        ...
        type:
          ...
          (result.assets[0].type === "video" ? "video/mp4" : "image/jpeg"),
      });
  };
  const save = async () => {
    if ((!text.trim() && !media) || saving) return;
    ...
    try {
      ...
      console.warn("[home] story creation failed", error);
    } finally {
      setSaving(false);
    }
  };
      ...
      >
        ...
        >
          ...
          <TextInput
            ...
            value={text}
            onChangeText={setText}
            placeholder="Write something for your circle"
            placeholderTextColor={theme.textSecondary}
            style={{
              minHeight: 90,
              color: theme.text,
...
function StoryViewer({
  ...
  const submitComment = async () => {
    ...
    try {
      await addStoryComment(story.id, comment);
      setComment("");
    } catch (error) {
      console.warn("[home] story comment failed", error);
    }
  };
  const like = async () => {
    try {
      ...
      console.warn("[home] story like failed", error);
    }
  };
  return (
      ...
      >
        ...
        >
          ...
          <View
            style={{
              padding: 22,
              backgroundColor: story.mediaUrl
                ? "rgba(0,0,0,0.45)"
          ...
          >
            ...
            <View style={{ flexDirection: "row", gap: 10 }}>
              ...
              >
                <Heart
                  size={20}
                  color={liked ? "#E85AAD" : "#fff"}
                  fill={liked ? "#E85AAD" : "transparent"}
                />
              </TouchableOpacity>
              <TextInput
                value={comment}
      ...
      >
        ...
        >
          ...
          {friends.length ? (
            ...
            >
              Connect with someone before sharing.
            </Text>
          )}
        </View>
      </View>
      ...
      <View style={styles.pulseCard}>
        <View style={styles.pulseHeader}>
          {item.avatar ? (
            ...
            <View
              style={[styles.authorAvatar, { backgroundColor: item.color }]}
            >
          ...
          <View style={styles.authorCopy}>
            ...
            <Text style={styles.pulseMeta}>
              {item.ago} · {item.channel}
            </Text>
          </View>
          <TouchableOpacity>
            <MoreHorizontal size={21} color={theme.textSecondary} />
          </TouchableOpacity>
        </View>
                ...
                <View>
                  <FlatList
                    ...
                    renderItem={({ item: uri }) => (
                      <Image
                        source={{ uri }}
                        style={styles.pulseImageCarousel}
                      />
                    )}
                  />
          ...
          <View style={styles.likedByRow}>
            ...
            <Text style={styles.likedByText} numberOfLines={1}>
              ...
              {item.likedBy.length > 1 && (
                <Text>
                  {" "}
                  and <Text style={styles.likedByName}>{item.likedBy[1]}</Text>
                </Text>
              )}
        ...
        <View style={styles.reactions}>
          <View>
            ...
            <Animated.View style={{ transform: [{ scale: likeButtonScale }] }}>
              ...
              >
                {activeReaction ? (
                  <Text style={styles.reactionEmojiInline}>
          ...
          >
            <MessageCircle size={19} color={theme.textSecondary} />
            <Text style={styles.reactionText}>{item.comments}</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.reaction} onPress={onShare}>
            <Share2 size={18} color={theme.textSecondary} />
            <Text style={styles.reactionText}>Share</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.save}>
            <Bookmark size={19} color={theme.primary} />
          </TouchableOpacity>
        </View>
            ...
            <TouchableOpacity
              ...
              onPress={async () => {
                ...
                try {
                  ...
                  console.warn("[home] comment failed", error);
                }
              }}
              style={{
                paddingHorizontal: 14,
...
const createStyles = (theme: ReturnType<typeof useTheme>) =>
  StyleSheet.create({
    ...
    feed: { paddingBottom: BottomTabInset + 34 },
    header: {
      minHeight: 66,
    ...
    inboxBadgeText: { color: "#fff", fontFamily: Fonts?.sansBold, fontSize: 9 },
    actionRow: {
      marginHorizontal: 20,
      marginTop: 10,
      flexDirection: "row",
    ...
    stories: { paddingHorizontal: 20, paddingTop: 13, gap: 14 },
    story: { width: 61, alignItems: "center", position: "relative" },
    storyRing: {
      width: 56,
      height: 56,
      borderRadius: 20,
    ...
    storyInitials: { color: "#fff", fontFamily: Fonts?.sansBold, fontSize: 13 },
    storyName: {
      color: theme.textSecondary,
      fontFamily: Fonts?.sansMedium,
      fontSize: 10,
    ...
    shareAvatarImage: { width: "100%", height: "100%" },
    shareAvatarText: {
      color: "#fff",
      fontFamily: Fonts?.sansBold,
      fontSize: 11,
    ...
    authorAvatarImage: { width: 40, height: 40, borderRadius: 15 },
    authorInitials: {
      color: "#fff",
      fontFamily: Fonts?.sansBold,
      fontSize: 12,
    ...
    reactionEmojiInline: { fontSize: 16 },
