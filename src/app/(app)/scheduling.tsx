...
export default function SchedulingScreen() {
  ...
  const [scheduleTimeZone, setScheduleTimeZone] = useState(getDeviceTimeZone);
  ...
  const loadData = useCallback(async () => {
    try {
      const [scheduleRows, recipientRows] = await Promise.all([
        getScheduledMessages(),
        getScheduleRecipients(),
      ]);
  ...
  useRealtimeRecovery(loadData);

  useEffect(() => {
    const channel = supabase
      .channel("scheduled-message-updates")
      .on(
        "postgres_changes",
      ...
      .subscribe((status) => {
        if (status === "SUBSCRIBED") void loadData();
      });
    return () => {
  ...
  const resetComposer = () => {
    ...
    setScheduleTimeZone(getDeviceTimeZone());
    setTotalDays(1);
  };
  ...
  const openComposer = (existing?: ScheduledMessageRecord) => {
    if (existing) {
      ...
      // Editing adopts the device's current zone so pre-timezone schedules are repaired when resaved.
      setScheduleTimeZone(getDeviceTimeZone());
  ...
  const handleSave = async () => {
    if (!selected)
      return Alert.alert(
        "Choose a recipient",
        "Select the person who should receive this message.",
      );
    if (!content.trim())
      return Alert.alert(
        "Write a message",
        "The scheduled message cannot be empty.",
      );
    if (scheduledFor <= new Date())
      return Alert.alert(
        "Choose a future time",
        "The first delivery must be in the future.",
      );
    setSaving(true);
    try {
    ...
    <SafeAreaView style={styles.safeArea} edges={["top"]}>
      ...
      >
        <View style={styles.header}>
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.subtitle}>
              Write it now. We will deliver it while you are busy.
            </Text>
          </View>
          <View style={styles.headerIcon}>
            <CalendarClock size={24} color={theme.primary} />
          </View>
        </View>
        ...
        >
          ...
          <Text style={styles.newButtonText}>Create schedule</Text>
        </TouchableOpacity>
        ...
        >
          {(["all", "pending", "recurring", "sent", "failed"] as Filter[]).map(
            (value) => (
              <TouchableOpacity
                key={value}
        ...
        {loading ? (
          ...
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>
              Your saved and delivered schedules will appear here.
            </Text>
          </View>
        ) : (
          visible.map((item) => {
            ...
            return (
              ...
              >
                <View style={styles.cardHeader}>
                  <View style={styles.avatar}>
                    <Text style={styles.avatarText}>
                      {item.recipient_name.slice(0, 1).toUpperCase()}
                    </Text>
                  </View>
                  <View style={{ flex: 1 }}>
                ...
                <Text style={styles.progress}>
                  {item.days_sent} of {item.total_days} deliveries sent
                </Text>
                {item.last_error ? (
                  <Text style={styles.errorText}>
                    {item.status === "failed"
                      ? "Delivery failed"
                      : `Retrying automatically (${item.retry_count}/5)`}
                    : {item.last_error}
                  </Text>
                ) : null}
              </TouchableOpacity>
      ...
      >
        <SafeAreaView style={styles.modalSafe}>
          ...
          >
            ...
            >
              <UserRound size={19} color={theme.textSecondary} />
              <Text
                style={[
                  styles.selectorText,
            ...
            <View style={styles.stepper}>
              ...
              <View style={styles.stepValue}>
                ...
                <Text style={styles.stepSub}>{scheduledFor.getFullYear()}</Text>
              </View>
              <TouchableOpacity
                style={styles.stepButton}
                onPress={() => adjustDate(1)}
            ...
            <View style={styles.stepper}>
              ...
              <View style={styles.stepValue}>
                ...
                <Text style={styles.stepSub}>
                  Daily in {scheduleTimeZone.replaceAll("_", " ")}
                </Text>
              </View>
              <TouchableOpacity
                style={styles.stepButton}
            ...
            <View style={styles.stepper}>
              <TouchableOpacity
                style={styles.stepButton}
                onPress={() => setTotalDays((value) => Math.max(1, value - 1))}
              >
                <Minus size={20} color={theme.primary} />
              </TouchableOpacity>
              <View style={styles.stepValue}>
                <Text style={styles.stepMain}>{totalDays}</Text>
                <Text style={styles.stepSub}>
                  {totalDays === 1 ? "send once" : "send once daily"}
                </Text>
              </View>
              <TouchableOpacity
                style={styles.stepButton}
                onPress={() =>
                  setTotalDays((value) => Math.min(365, value + 1))
                }
              >
                <Plus size={20} color={theme.primary} />
              </TouchableOpacity>
      ...
      <Modal
        ...
        transparent
        animationType="slide"
        onRequestClose={() => setRecipientPickerVisible(false)}
      >
            ...
            <ScrollView style={{ maxHeight: 390 }}>
              {filteredRecipients.length === 0 ? (
                <Text style={styles.noFriends}>
                  No friends available. Add a friend before scheduling a
                  message.
                </Text>
              ) : (
      ...
      >
        <Pressable style={styles.overlay} onPress={() => setActive(null)}>
          ...
          >
            {active ? (
              <>
                <Text style={styles.sheetTitle}>{active.recipient_name}</Text>
                ...
                {active.status !== "sent" ? (
                  <>
                    {active.days_sent === 0 ? (
                      <TouchableOpacity
                        style={styles.action}
                        onPress={() => openComposer(active)}
                      >
                        <Edit3 size={19} color={theme.text} />
                    ...
                    >
                      {active.paused ? (
                        <Play size={19} color={theme.text} />
                      ) : (
                        <Pause size={19} color={theme.text} />
                      )}
                    ...
                    <TouchableOpacity
                      ...
                      onPress={() =>
                        void perform(
                          () => sendScheduledMessageNow(active.id),
                          "Could not send message",
                        )
                      }
                    >
                      <Send size={19} color={theme.primary} />
                      <Text
                ...
                <TouchableOpacity
                  ...
                  onPress={() =>
                    void perform(
                      () => deleteScheduledMessage(active.id),
...
const createStyles = (theme: ReturnType<typeof useTheme>) =>
  StyleSheet.create({
    ...
    newButtonText: { color: "#fff", fontFamily: Fonts?.sansBold, fontSize: 15 },
    ...
    recipient: { color: theme.text, fontFamily: Fonts?.sansBold, fontSize: 14 },
    date: {
      color: theme.textSecondary,
      fontFamily: Fonts?.sans,
      fontSize: 11.5,
    ...
    done: { color: theme.primary, fontFamily: Fonts?.sansBold, fontSize: 14 },
    form: { padding: Spacing.four, paddingBottom: 60 },
    label: {
      color: theme.textSecondary,
      fontFamily: Fonts?.sansBold,
      fontSize: 11,
    ...
    stepMain: { color: theme.text, fontFamily: Fonts?.sansBold, fontSize: 17 },
    stepSub: {
      color: theme.textSecondary,
      fontFamily: Fonts?.sans,
      fontSize: 11,
